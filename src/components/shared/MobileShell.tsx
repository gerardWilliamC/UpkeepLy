...
import { Wordmark, Chip, TONES } from '@/components/ui';

// ─── FauxQR (needed by Viewfinder) ───────────────────────────────────────────
...
function FauxQR({ value = 'UPKEEPLY', size = 140, fg = 'var(--stone-900)', bg = '#fff' }: FauxQRProps) {
  const N = 25;
...
export function MobileScreen({ children, statusbar = true, tone = 'light', style = {} }: MobileScreenProps) {
  return (
    <div style={{
...
export function MobileStatusBar({ tone = 'light' }: MobileStatusBarProps) {
...
export function MButton({ children, variant = 'primary', icon, onClick, disabled, style = {} }: MButtonProps) {
  ...
  return (
    <button onClick={disabled ? undefined : onClick} disabled={disabled} style={{
      ...(variants[variant] || variants.primary), width: '100%', minHeight: 54, padding: '0 20px',
...
export function Viewfinder({ caption = 'Point at the zone QR sticker', detected = false }: ViewfinderProps) {
  return (
    <div style={{
...
export function Sheet({ children, onClose, height = 'auto' }: SheetProps) {
  return (
...
export function MHeader({ title, sub, onBack, tone = 'light', right }: MHeaderProps) {
...
interface MobileLandingProps {
  ...
  zone: { name: string; building: string; id: string };
}
export function MobileLanding({ onPick, zone }: MobileLandingProps) {
  const ROLES = [
    ...
    <>
      <div style={{ background: 'linear-gradient(135deg, #2B0000 0%, #6A0B0C 55%, #8C0001 100%)', padding: '6px 22px 30px', color: '#fff', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <span style={{ fontSize: 10.5, fontFamily: 'var(--font-mono)', color: 'rgba(255,255,255,0.55)' }}>LPU Cavite</span>
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '22px 22px 24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {ROLES.map(r => (
            ...
            onMouseEnter={e => { const el = e.currentTarget as HTMLDivElement; el.style.boxShadow = 'var(--shadow-md)'; el.style.transform = 'translateY(-1px)'; el.style.borderColor = 'var(--crimson-300)'; }}
            onMouseLeave={e => { const el = e.currentTarget as HTMLDivElement; el.style.boxShadow = 'var(--shadow-xs)'; el.style.transform = 'none'; el.style.borderColor = 'var(--border-subtle)'; }}>
