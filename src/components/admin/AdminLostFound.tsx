...
import { Button, Card, Chip, Eyebrow, H } from '@/components/ui';
import { FilterBar, Scrim } from '@/components/admin/AdminViews';
import { ago } from '@/components/ui';
import { LF_ICON } from '@/lib/mock-data';

const LF_STATE_TONE: Record<string, string> = { unclaimed: 'neutral', pending: 'warning', claimed: 'success', pmu: 'brand' };
const LF_STATE_LABEL: Record<string, string> = { unclaimed: 'Unclaimed', pending: 'Claim pending', claimed: 'Returned', pmu: 'PMU office only' };
...
interface LFItem {
  ...
  foundMin: number;
  by: string;
  claim?: { desc: string; email: string; min: number };
}
...
function ClaimReviewModal({ item, onClose, onResolve }: ClaimReviewModalProps) {
  const [sent, setSent] = React.useState(false);
  const email = `Subject: Your lost item has been verified — ${item.ref}

Good day,

The item you reported — "${item.name}" — has been matched to a found item in the LPU Cavite Lost & Found system.
...
interface LostFoundAdminProps {
  items: LFItem[];
  onResolveClaim: (id: string, decision: string) => void;
  onLog: () => void;
}

export function LostFoundAdmin({ items, onResolveClaim, onLog }: LostFoundAdminProps) {
  const [filter, setFilter] = React.useState('All');
  const [claim, setClaim] = React.useState<LFItem | null>(null);
  const filters = ['All', 'Claim pending', 'Unclaimed', 'Returned', 'PMU office only'];
  const map: Record<string, string> = { 'Claim pending': 'pending', 'Unclaimed': 'unclaimed', 'Returned': 'claimed', 'PMU office only': 'pmu' };
            ...
            <Card key={it.id} pad={0} hover style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              <div style={{ padding: 18, display: 'flex', gap: 14, flex: 1 }}>
                <div style={{ width: 48, height: 48, borderRadius: 10, background: 'var(--bg-sunken)', color: 'var(--fg-2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Icon name={(LF_ICON as Record<string, string>)[it.category] || 'package'} size={22} />
                </div>
