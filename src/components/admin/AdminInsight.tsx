...
import { Card, Chip, Eyebrow, H, TONES } from '@/components/ui';
import { Button } from '@/components/ui';
import { SERIES_14D, HEATMAP, HEAT_HOURS } from '@/lib/mock-data';

function BarChart({ data, labels, h = 150 }: { data: number[]; labels: string[]; h?: number }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
      <svg width={140} height={140} viewBox="0 0 140 140">
        ...
        <text x="70" y="66" textAnchor="middle" style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 26, fill: 'var(--fg-1)' } as React.CSSProperties}>{total}</text>
        <text x="70" y="84" textAnchor="middle" style={{ fontSize: 10, fill: 'var(--fg-3)', letterSpacing: '0.1em' } as React.CSSProperties}>TOTAL</text>
      </svg>
...
export function AnalyticsView() {
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: 28 }}>
      ...
      <Card>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', borderBottom: '1px solid var(--border-subtle)', paddingBottom: 12, marginBottom: 16 }}>
          <div><Eyebrow style={{ marginBottom: 5 }}>Campus Heatmap</Eyebrow><H level={3}>Issue intensity by building × hour</H></div>
            ...
            {HEATMAP.map(row => (
              <React.Fragment key={row.building}>
                ...
                {row.hours.map((v, i) => (
                  ...
                  onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.transform = 'scale(1)'}>{v > 0 ? v : ''}</div>
                ))}
              </React.Fragment>
            ))}
          </div>
