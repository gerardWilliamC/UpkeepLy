...
import { Card, Chip, Eyebrow, TONES, STATUS_TONE, zoneIcon, ago } from '@/components/ui';
import { Button } from '@/components/ui';
import { SERIES_14D } from '@/lib/mock-data';
...
interface KpiCardProps { icon: string; label: string; value: number; delta?: number; tone?: string; spark?: number[]; }
export function KpiCard({ icon, label, value, delta, tone = 'brand', spark }: KpiCardProps) {
...
interface LogRowProps { log: LogItem; isNew?: boolean; onSelect?: (log: LogItem) => void; }
export function LogRow({ log, isNew, onSelect }: LogRowProps) {
...
interface AlertRowProps { alert: AlertItem; onAssign: (alert: AlertItem) => void; }
export function AlertRow({ alert, onAssign }: AlertRowProps) {
  const pr = ({ High: 'danger', Medium: 'warning', Low: 'neutral' } as Record<string, string>)[alert.priority] ?? 'neutral';
  return (
...
interface AdminDashboardProps {
  ...
  alerts: AlertItem[];
  newIds: Set<string>;
  kpi: { verifications: number; cleanings: number; activeWorkers: number };
  onAssign: (alert: AlertItem) => void;
  onNavigate?: (view: string) => void;
  live: boolean;
  onToggleLive: () => void;
}

export function AdminDashboard({ logs, alerts, newIds, kpi, onAssign, onNavigate: onNav, live, onToggleLive }: AdminDashboardProps) {
