...
export function useTweaks<T extends Record<string, unknown>>(defaults: T): [T, (keyOrEdits: string | Partial<T>, val?: unknown) => void] {
  ...
  const setTweak = React.useCallback((keyOrEdits: string | Partial<T>, val?: unknown) => {
...
interface TweakRowProps { label: string; value?: React.ReactNode; children?: React.ReactNode; inline?: boolean; }
...
type TweakOption = string | number | boolean | { value: unknown; label: string };
interface TweakSelectProps { label: string; value: unknown; options: TweakOption[]; onChange: (v: string) => void; }
  ...
  return (
    <TweakRow label={label}>
      <select className="twk-field" value={String(value)} onChange={(e) => onChange(e.target.value)}>
        {options.map((o, i) => {
          const v = typeof o === 'object' && o !== null && 'value' in o ? String((o as { value: unknown }).value) : String(o);
          const l = typeof o === 'object' && o !== null && 'label' in o ? (o as { label: string }).label : String(o);
...
export function TweakRadio({ label, value, options, onChange }: TweakRadioProps) {
  ...
  const labelLen = (o: TweakOption) => String(typeof o === 'object' && o !== null && 'label' in o ? (o as { label: string }).label : o).length;
  const maxLen = options.reduce((m: number, o: TweakOption) => Math.max(m, labelLen(o)), 0);
  const fitsAsSegments = maxLen <= ({ 2: 16, 3: 10 }[options.length] ?? 0);

  if (!fitsAsSegments) {
    const resolve = (s: string) => {
      const m = options.find((o) => String(typeof o === 'object' && o !== null && 'value' in o ? (o as { value: unknown }).value : o) === s);
      return m === undefined ? s : typeof m === 'object' && m !== null && 'value' in m ? (m as { value: unknown }).value : m;
    };
    return <TweakSelect label={label} value={value} options={options} onChange={(s) => onChange(resolve(s))} />;
  }

  const opts = options.map((o) => (typeof o === 'object' && o !== null && 'value' in o ? o as { value: unknown; label: string } : { value: o, label: String(o) }));
...
interface TweakColorProps { label: string; value: ColorOption; options?: ColorOption[]; onChange: (v: ColorOption) => void; }
