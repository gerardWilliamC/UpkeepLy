...
export const CAMPUSES = [
  { id: 'main',   name: 'LPU Cavite — Main Campus',   short: 'Main Campus',  zones: 148, workers: 34, code: 'LPU-01' },
  { id: 'north',  name: 'LPU Cavite — North Annex',    short: 'North Annex',  zones: 62,  workers: 14, code: 'LPU-02' },
  { id: 'river',  name: 'LPU Cavite — Riverside Ext.', short: 'Riverside',    zones: 41,  workers: 9,  code: 'LPU-03' },
];

export const BUILDINGS = ['Rizal Hall', 'Engineering Complex', 'Science Wing', 'Student Center', 'Main Library', 'Sports Complex'];
...
export const SEED_ALERTS = [
  { id: 'A-2051', zone: 'Restroom · GF North',  zoneId: 'Z-0520', building: 'Main Library',         status: 'Open',     priority: 'High',   min: 12,  note: 'Tap leaking, floor flooding near stalls 3–4.', source: 'Public report', assignee: undefined },
  { id: 'A-2050', zone: 'Cafeteria Hall A',     zoneId: 'Z-0401', building: 'Student Center',       status: 'Open',     priority: 'Medium', min: 22,  note: 'Spill near counter 3 — signage placed, mop needed.', source: 'Marisol Reyes', assignee: undefined },
  { id: 'A-2049', zone: 'Exterior Light · Path 4', zoneId: 'Z-0820', building: 'Sports Complex',    status: 'Open',     priority: 'High',   min: 56,  note: 'Walkway light out — dark stretch near field gate.', source: 'Public report', assignee: undefined },
...
export const LOST_FOUND = [
  ...
  { id: 'LF-7780', name: 'iPhone 14, navy case',    category: 'Electronics', found: 'Cafeteria Hall A',    foundMin: 140,  by: 'Dann Villanueva',state: 'pending',   public: true,  ref: 'LF-7780', desc: 'Navy silicone case, cracked top-left corner.', claim: { email: 'a.santos@lpu.edu.ph', desc: 'Lock screen is a photo of a corgi. Cracked corner near camera.', min: 40 } },
  { id: 'LF-7779', name: 'Student ID — J. Domingo', category: 'IDs',         found: 'Rizal Hall · 2F',     foundMin: 200,  by: 'Marisol Reyes', state: 'unclaimed', public: true,  ref: 'LF-7779', desc: 'LPU Cavite ID card, name J. Domingo, BS-CpE.' },
...
export const LF_CATEGORIES = ['All', 'Bags', 'Electronics', 'IDs', 'Wallets', 'Clothing'];

export const LF_ICON: Record<string, string> = {
  Bags: 'package', Electronics: 'smartphone', IDs: 'fingerprint',
  Wallets: 'banknote', Clothing: 'package', Cash: 'banknote', Keys: 'key',
};

export const WORKER_PROFILE = { id: 'W-1063', name: 'Grace Lim', initials: 'GL', color: '#2F7D4F', shift: 'AM · 06:00–14:00', zonesAssigned: 14 };
...
export function checklistFor(type: string): string[] { return CHECKLISTS[type] || CHECKLISTS._default; }
...
export const WORKER_HISTORY_SEED = [
...
export const SERIES_14D = [218, 240, 233, 261, 198, 142, 130, 255, 271, 248, 290, 277, 305, 312];
export const HEATMAP = BUILDINGS.map((b, i) => ({
  building: b,
...
export const HEAT_HOURS = ['6a','7a','8a','9a','10a','11a','12p','1p','2p','3p','4p','5p','6p','7p','8p'];
