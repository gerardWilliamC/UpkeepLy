// =========================================================================
// Upkeeply · Public mobile · Lost & Found board + claim flow
// =========================================================================

function PublicLostFound({ onBack, items: extraItems = [], onSubmitItem }) {
  const [cat, setCat] = React.useState('All');
  const [claimItem, setClaimItem] = React.useState(null);
  const [query, setQuery] = React.useState('');
  const [reporting, setReporting] = React.useState(false);
  // public board = seed public items + anything submitted this session, never PMU-only
  ...
  return (
    <>
      <MHeader title="Lost &amp; Found" sub="Northgate University · public board" onBack={onBack}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '6px 18px 24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {items.map(it => (
            <div key={it.id} style={{ display: 'flex', gap: 13, padding: 14, background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 14.5, fontWeight: 600, color: 'var(--fg-1)' }}>{it.name}</div>
                <div style={{ fontSize: 12, color: 'var(--fg-3)', marginTop: 3, display: 'flex', alignItems: 'center', gap: 5 }}>
                  <Icon name="map-pin" size={12} />{it.found} · {ago(it.foundMin)}
                </div>
...
// ─── Anonymous self-submission: report a found OR lost item ───────────────────
function ReportItemSheet({ onClose, onSubmit }) {
  const [kind, setKind] = React.useState('found'); // found | lost
  const [name, setName] = React.useState('');
  const [category, setCategory] = React.useState('Bags');
  const [place, setPlace] = React.useState('');
  const [desc, setDesc] = React.useState('');
  const [email, setEmail] = React.useState('');
  const [done, setDone] = React.useState(null); // ref string
    ...
    <Sheet onClose={onClose}>
      <div style={{ textAlign: 'center', padding: '12px 4px 8px' }}>
        ...
        <div style={{ fontSize: 14, color: 'var(--fg-2)', lineHeight: 1.55, maxWidth: 290, margin: '0 auto' }}>
          {kind === 'found'
            ? <>Thank you. Please drop the item at the PMU office, Student Center GF. It will appear on the board once staff log it in.</>
    ...
    <Sheet onClose={onClose}>
      ...
      <Field label="Category">
        <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
          {cats.map(c => {
            const on = category === c;
            return <button key={c} onClick={() => setCategory(c)} style={{ padding: '7px 13px', borderRadius: 999, cursor: 'pointer', fontSize: 12.5, fontWeight: 600, fontFamily: 'var(--font-sans)', border: `1px solid ${on ? 'var(--crimson-600)' : 'var(--border-subtle)'}`, background: on ? 'var(--crimson-600)' : 'var(--bg-surface)', color: on ? '#fff' : 'var(--fg-2)' }}>{c}</button>;
          })}
        </div>
      </Field>
      ...
      <Field label={kind === 'found' ? 'Describe it (helps verify the owner)' : 'Distinguishing details'}>
      ...
      <Field label="Your email">
        <input value={email} onChange={e => setEmail(e.target.value)} placeholder="you@northgate.edu" style={lfInput} />
      </Field>
      <div style={{ height: 4 }} />
...
function ClaimSheet({ item, onClose }) {
  const [email, setEmail] = React.useState('');
  const [desc, setDesc] = React.useState('');
  ...
  if (sent) return (
    <Sheet onClose={onClose}>
      <div style={{ textAlign: 'center', padding: '12px 4px 8px' }}>
        ...
        <div style={{ marginTop: 18, padding: '9px 16px', background: 'var(--bg-sunken)', borderRadius: 999, display: 'inline-block', fontFamily: 'var(--font-mono)', fontSize: 12.5, color: 'var(--fg-2)' }}>Ref · {item.ref}</div>
        <div style={{ marginTop: 22 }}><MButton onClick={onClose}>Done</MButton></div>
      </div>
    </Sheet>
  );
  ...
  return (
    <Sheet onClose={onClose}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
        <div style={{ width: 44, height: 44, borderRadius: 10, background: 'var(--bg-sunken)', color: 'var(--fg-2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon name={LF_ICON[item.category] || 'package'} size={20} /></div>
        <div><div style={{ fontSize: 15.5, fontWeight: 600, color: 'var(--fg-1)' }}>{item.name}</div><div style={{ fontSize: 12, color: 'var(--fg-3)', fontFamily: 'var(--font-mono)' }}>{item.ref}</div></div>
      </div>
      ...
      <input value={email} onChange={e => setEmail(e.target.value)} placeholder="you@northgate.edu" style={{
        width: '100%', boxSizing: 'border-box', padding: '13px 14px', marginBottom: 16,
      ...
      <textarea value={desc} onChange={e => setDesc(e.target.value)} placeholder="A distinguishing detail — contents, marks, lock screen, engraving…" style={{
        width: '100%', height: 92, resize: 'none', boxSizing: 'border-box', padding: 14,
        fontFamily: 'var(--font-sans)', fontSize: 14.5, color: 'var(--fg-1)', lineHeight: 1.45,
        background: 'var(--bg-canvas)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-md)', marginBottom: 18,
      }} />
...
Object.assign(window, { PublicLostFound, ClaimSheet, ReportItemSheet, Field });
