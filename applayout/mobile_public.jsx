// =========================================================================
// Upkeeply · Public mobile flow (anonymous report + lost & found board)
// =========================================================================
...
// ─── Public entry: what you see after scanning a zone QR as a passer-by ──────
function PublicFlow({ onSubmitReport, onSubmitItem, lfExtra = [], onExit }) {
  // step: landing → report → photo → done  | or → lostfound board
  const [step, setStep] = React.useState('landing');
  const [mode, setMode] = React.useState('report'); // report | lostfound
    ...
    <>
      ...
      <div style={{ padding: '14px 22px', borderTop: '1px solid var(--border-subtle)', flexShrink: 0 }}>
        <MButton icon="send" disabled={!issue} onClick={() => {
          ...
          setRef(r);
          onSubmitReport({ zone: ZONE, issue: ISSUE_TYPES.find(i => i.id === issue), note });
          setStep('done');
...
Object.assign(window, { PublicFlow, ISSUE_TYPES });
